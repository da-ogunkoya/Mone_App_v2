...
//admin request for agent/customer
		if(isset($_GET['tableType'])){
			$table=$_GET['tableType']=='agent_cust_transaction'?'agent_cust_transaction':'cust_transaction';			
		}
//user request as agent/customer
		else{
			$table=(getUser()['type']=='agent' )?"agent_cust_transaction": "cust_transaction";
			}
			
			$this->db->query("SELECT * FROM $table WHERE id=:transId");
