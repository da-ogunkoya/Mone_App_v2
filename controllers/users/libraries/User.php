...
class User{
	 ...
	 */
	 ...
	 public function findUsers($searchTerm,$table,$id){
		 
		 if($table=='agent_new_customer'){
			$this->db->query("SELECT id,fname,lname FROM agent_new_customer WHERE (fname LIKE '%".$searchTerm."%' || lname LIKE '%".$searchTerm."%' || pnumber LIKE '%".$searchTerm."%' || mnumber LIKE '%".$searchTerm."%') && agid='$id' ORDER BY fname ASC");
			}
		else{
			$this->db->query("SELECT id,b_fname,b_lname FROM receiver WHERE (b_fname LIKE '%".$searchTerm."%' || b_lname LIKE '%".$searchTerm."%')&& cid='$id' ORDER BY b_fname ASC");
		}
	 ...
	 //search admin agents 
	 	 public function findAdminAgents($searchTerm){
			$this->db->query("SELECT id,fname,lname FROM agent WHERE (fname LIKE '%".$searchTerm."%' || lname LIKE '%".$searchTerm."%' || pnumber LIKE '%".$searchTerm."%' || mnumber LIKE '%".$searchTerm."%') ORDER BY fname ASC");
	  ...
	  //search admin customer 
	 	 public function findAdminCusts($searchTerm){
			$this->db->query("SELECT id,fname,lname FROM new_customer WHERE (fname LIKE '%".$searchTerm."%' || lname LIKE '%".$searchTerm."%' || pnumber LIKE '%".$searchTerm."%' || mnumber LIKE '%".$searchTerm."%') ORDER BY fname ASC");
			
			$row=$this->db->resultset();
		 return $row;
	 }
	 
	   //search admin agent customer 
	 	 public function findAdminAgentCusts($searchTerm){
			$this->db->query("SELECT id,fname,lname FROM agent_new_customer WHERE (fname LIKE '%".$searchTerm."%' || lname LIKE '%".$searchTerm."%' || pnumber LIKE '%".$searchTerm."%' || mnumber LIKE '%".$searchTerm."%') ORDER BY fname ASC");
	...
	// for admin
		...
		//Check Rows
		if($this->db->rowCount() > 0){
			$this->setUserDataAdmin($row,$data);
			return true;
		} else {
			return false;
		}
	 ...
	 */
	private function setUserDataAdmin($rowad,$data){
	...
	$_SESSION['passt']=$data['password'];
	$_SESSION['level']= $rowad->level;
	$_SESSION['web1']=binfo()['web1'];
