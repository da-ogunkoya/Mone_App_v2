...
class sendMoney{
	...
	public function getSenderinfo($senderid){
		$this->db->query("SELECT * FROM agent_new_customer WHERE id=:senderid");
		$this->db->bind(':senderid',$senderid);
		$result=$this->db->resultset();
			foreach($result as $result){
				$name=$result->fname." ".$result->lname." ".$result->mname;
			}
		return $name;
		
	}
	
	public function getReceiverinfo($receiverid){
		if(getUser()['type']=='agent'){$table="agent_cust_receiver";} else{ $table="receiver";}
		$this->db->query("SELECT * FROM $table WHERE id=:receiverid");
	...
	public function sendingStatus(){
		if(getUser()['type']=='agent'){$table="agent";} else {$table="new_customer";}
		$userid=getUser()['user_id'];
		$table="SELECT active,id FROM `$table` WHERE id=:id && active=-1";
		$this->db->query($table);
	...
	
	{
		...
		$table="SELECT fname,mname,lname,proofid_size,proofad_size FROM agent_new_customer WHERE (fname=:sFName && mname=:sMName && lname=:sLName) && (proofid_size='0' ||proofad_size='0')";	
		}
		
		else{
		$table="SELECT fname,mname,lname,proofid_size,proofad_size FROM new_customer WHERE (fname=:sFName && mname=:sMName && lname=:sLName) && (proofid_size='0' ||proofad_size='0')";		
		}
		...
		if ($ecredit==1){
			if($agentCredit>=$amount){
				...
				//if successfully submitted go back to send money page
						...
						$trans=$this->submitTransaction($required);
						$id=$trans[1];
						$string="transId=".$id;
				//the return is in array, which return id and bolean true or false	
					if($trans[0]){
						redirectValue('receipt.php','New Transaction Successfully  Submitted','success',$string);
					}
	...
	//if successfully submitted go back to receipt page
			$trans=$this->submitTransaction($required);
						$id=$trans[1];
						$string="transId=".$id;
	//the return is in array, which return id and bolean true or false	
	...
	public function tcode(){
		...
		private function submitTransaction($required){
			...
			if(getUser()['type']=='agent'){
				$table="INSERT INTO agent_cust_transaction(date,receiptno,agent_email,sender_email,sender_name,s_phone,r_name,r_phone,
							...
							VALUE(:dDate,:tcode,:agentEmail,:senderEmail,:senderName,:senderPhone,:receivers,:rPhone,
							  ...
							  :county,:country,:uc,:rId,:senderId,:agid)";
			}
				else{
					$table="INSERT INTO cust_transaction(date,receiptno,sender_email,sender_name,s_phone,r_name,r_phone,
							r_transfer,r_bank,r_idtype,
							amt_send,total,amt_local,commission,com_a,com_d,status, exchange_rate,r_actno,
							dtime,crid,cid,clear)

							VALUE(:dDate,:tcode,:senderEmail,:senderName,:senderPhone,:receivers,:rPhone,
							  :tOption,:rBank,:rIdType,:amount,:tAmount,:lAmount,:commission,:aCom,:bCom,
							  :status,:eRate,:rActno,:dateTime,:rId,:senderId,:uc)";
				}
				$this->db->query($table);
								...
								$this->db->bind(':senderId',$required['senderId'] );
								$this->db->bind(':rId',$required['rId'] );
