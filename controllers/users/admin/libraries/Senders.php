...
class Senders{
	...
	//List All Agent Senders
		...
		public function update($listArray,$agcid){
			$table="UPDATE agent_new_customer SET fname=:fname,lname=:lname,address=:address,postcode=:postcode,country=:country,email=:email,pnumber=:contact, mnumber=:mobile ,proofid_name=:proofId_name , proofid_size=:proofId_size,proofid_type=:proofId_type,proofad_name=:proofad_name,proofad_size=:proofad_size,proofad_type=:proofad_type WHERE id='$agcid'";
			$this->db->query($table);
			...
			$this->db->bind(':country',$listArray['country']);
