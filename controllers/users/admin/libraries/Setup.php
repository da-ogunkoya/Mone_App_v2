...
//agent commission table-Display			
			public function comTableAgent($id){
				$table=$id=="none"?"SELECT * FROM agent_cr WHERE status='' ORDER BY `from` DESC":"SELECT * FROM agent_cr WHERE status='$id' ORDER BY `from` DESC";
				$this->db->query($table);
				$row=$this->db->resultset();
				return $row;
			}
//customer commission -Display			
...
//by id
					...
					if(isset($_GET['Id'])){
						$id=$_GET['Id'];
						$this->db->query("SELECT * FROM  `agent_cr` WHERE `to` =:amountTo AND  `from` =:amountFrom AND status='$id'");
	...
	//agent & customer				
							...
							if(isset($_GET['type'])){
									...
									if($type=='c'){
										...
										$this->db->query("SELECT * FROM  `agent_cr` WHERE `to` =:amountTo AND  `from` =:amountFrom AND status=''");	
									}
							}
							else{
								$this->db->query("SELECT * FROM  `agent_cr` WHERE `to` =:amountTo AND  `from` =:amountFrom AND status='' ");
...
//insert new commission range			
			public function newRange($arrayValue,$id){
					$dateTime=dateTime();
//set GET id
			if(isset($_GET['Id'])){
				$this->db->query("INSERT INTO `agent_cr`(`from`,`to`,`value`,`pa`,`status`,`date`) VALUES(:amountFrom,:amountTo,:value,:rate,'$id','$dateTime') ");
			}
...
//new business setup submission
	public function newSubmission($arrayValue){
		...
		$this->db->query("INSERT INTO busy(name1,name2,slogan1,slogan2,email,email2,tel1,tel2,tel3,web1,web2,fax,address,postcode,country,paypal_status,paypal_email,paypal_ipn,sms_status,sms_email,sms_hash,code,size,ecredit,br,mtls,dtls,vdtl,mtl,dtl,date) VALUE(:name1,:name2,:slogan1,:slogan2,:email,:email2,:tel1,:tel2,:tel3,:web1,:web2,:fax,:address,:postcode,:country,:paypalStatus,:paypalEmail,:paypalLimit,:smsStatus,:smsEmail,:smsHash,:businessCode,:businessType,:eCredit,:businessNameOnReceipt,:maxTransaction,:dailyLimit,:activateId,:maxValue,:limitValue,'$dateTime')");
