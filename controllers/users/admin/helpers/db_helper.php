	 ...
	 //get category of todays rate from agent database 
		...
		function binfo(){
			...
			foreach($result as $result);{
				...
				$buinfo['so']=$result->so;
				$buinfo['rt']=$result->rt;
				$buinfo['size']=$result->size;
				$buinfo['smsEmail']=$result->sms_email;
				$buinfo['smsHash']=$result->sms_hash;
		...
		function pTable(){
			...
			switch($bsize){
				case 1:
				$bTable['userTable']='new_customer';
				$bTable['transTable']='cust_transaction';
				$bTable['type']='c';
				break;
				
				case 2:
				$bTable['userTable']='agent';
				$bTable['transTable']='agent_cust_transaction';
				$bTable['type']='a';
				break;
