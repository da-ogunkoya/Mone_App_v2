...
//search 
		if(isset($_GET['searchUser'])){
			...
			$getId =substr($searchUser, strpos( $searchUser, "-") + 1);
		}
...
// List  agent customer	
		...
		$table="SELECT a.id,a.fname,a.lname,a.title,a.mnumber FROM agent_new_customer a
	...
	//search	
		if(!isset($getId)){
				$table="SELECT a.id,a.fname,a.lname,a.title,a.mnumber,(SELECT COUNT(id) FROM agent_cust_transaction I3 WHERE I3.agcid=a.id)as countTrans  FROM agent_new_customer a
			...
			$table="SELECT a.*,(SELECT COUNT(id) FROM agent_cust_transaction I3 WHERE I3.agcid=a.id)as countTrans  FROM agent_new_customer a
				WHERE a.agid='$user_id' && a.id='$getId' ORDER BY a.date_reg DESC LIMIT $start,$end";
		}
...
// List  customer	Receiver
	...
	$user_id=getUser()['user_id'];
		$table="SELECT a.* FROM receiver a
		...
		$pages = new Paginator($table,9,array(15,3,6,9,12,25,50,100,250,'All'));
			...
			if(!isset($getId)){
				...
				$table="SELECT a.*,(SELECT COUNT(id) FROM cust_transaction I3 WHERE I3.crid=a.id)as countTrans  FROM receiver a
				WHERE a.cid='$user_id' && id='$getId' ORDER BY a.date DESC LIMIT $start,$end";
			}
		$template->trans_result=$cust->custList($table);// display table result
		$template->pagin= $pages->display_pages();														//quick menu
		$template->pages=$pages->display_jump_menu(); $template->items=$pages->display_items_per_page();	//quick menu
	...
	$user_id=getUser()['user_id'];
		$table="SELECT a.*  FROM agent_cust_receiver a
