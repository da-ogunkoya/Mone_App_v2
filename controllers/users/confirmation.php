...
$sendMoney=new sendMoney;
	...
	if(isset($_POST['Confirm'])){
		//need sender address, sender phone,sender email,r_idtype
			...
			//$required['tAmount']=(float)$_GET['tAmount'];
			$required['tAmount'] =(float)$required['amount'] + (float)$required['commission'];
			$lAmount=explode(".",($_GET['lAmount']));$lAmount=$lAmount[0];$required['lAmount'] = str_replace(',', '', $lAmount);
			//$required['lAmount'] = (float)$_GET['amount'] * (float)$_GET['eRate'];
