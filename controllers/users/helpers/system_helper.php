...
//date 7 october 2016
function dateOnly($nDate){
	$newDate = date("j  F  Y", strtotime($nDate));
	return $newDate;
}

//time 7 october 2016
function timeOnly($nTime){
	 $newTime = date("h:i:A", strtotime($nTime));
	return $newTime;
}
...
function systemCheck(){
	$idletime=520;//after 60 seconds the user gets logged out
	recordActivities();
...
function system_keys(){
    $db = new Database;
    $db->query("SELECT * FROM api ORDER BY id DESC LIMIT 1");
    
    $result = $db->single();
    return array(
        'facebook'=>$result->facebook_login,
        'google'=>$result->google_login,
        'postcode'=>$result->google_postcode
    );
