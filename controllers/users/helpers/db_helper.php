...
function userPostCount($user_id){
	...
	function subscription(){
		$db = new Database;
		$db->query("SELECT `due_date`,`amount_due` FROM `subscription`  ORDER BY `id` DESC 
                        LIMIT 1 ");
		$result=$db->resultset();
		...
		$dateFetch = $dateSearch->format('Y-m-d');

		$now = time(); // or your date as well
		$your_date = strtotime($dateFetch);
		$datediff = $now - $your_date;

		$date =  round($datediff / (60 * 60 * 24));
		return ['daysleft'=>$date,'amount_due'=>$amount_due];
